# This Makefile is for the Unzip::Passwd extension to perl.
#
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Unzip/Passwd.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Unzip
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Unzip
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Unzip/Passwd.pm $(INST_MAN3DIR)/Unzip::Passwd.$(MAN3EXT) 
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0.0.14">' > $(DISTNAME).ppd
